...
resource "azurerm_private_endpoint" "sql_private_endpoint"{
    ...
    //como se conectará a la base de datos
    private_service_connection{
        name = "sql-private-ec-${var.project }-${var.environment}"
        private_connection_resource_id = azurerm_mssql_server.sql_server.id

        //especifica los servicios que van a estar corriendo dentro
...
resource "azurerm_private_dns_a_record" "private_dns_a_record"{
    ...
    //ip address que queremos agregar
...
resource "azurerm_sql_firewall_rule" "allow_my_ip"{
    ...
    start_ip_address = "131.221.8.100"
    end_ip_address = "131.221.8.100"
}